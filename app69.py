...
class ResearchCategory(Enum):
    CASE_STUDIES = "Case Studies and Precedents"
    IP_LANDSCAPE = "Intellectual Property Landscape"
    GEO_CULTURAL = "Geographic and Cultural Analysis"
    MARKET_INSIGHTS = "Market and Consumer Insights"
    COMPETITIVE_INTEL = "Competitive Intelligence"
    EMERGING_TECH = "Emerging Technologies and Trends"
    ECONOMIC_POLITICAL = "Economic and Political Factors"
    SUSTAINABILITY = "Sustainability and Environmental Impact"
    FUNDING_INVESTMENT = "Funding and Investment Landscape"
    WORKFORCE_TALENT = "Workforce and Talent Analysis"
    REGULATORY_COMPLIANCE = "Regulatory and Compliance Research"
    CUSTOMER_EXPERIENCE = "Customer Experience and Service"
    SUPPLY_CHAIN = "Supply Chain and Logistics"
    CROSS_INDUSTRY = "Cross-Industry Applications"
    LONG_TERM_TRENDS = "Long-term Trend Analysis"
    DIGITAL_TRANSFORMATION = "Digital Transformation Strategies"
    CYBERSECURITY = "Cybersecurity and Data Protection"
    SOCIAL_MEDIA_TRENDS = "Social Media and Online Presence Analysis"
    DIVERSITY_INCLUSION = "Diversity, Equity, and Inclusion Practices"
    REMOTE_WORK = "Remote Work and Distributed Team Management"
    CIRCULAR_ECONOMY = "Circular Economy and Waste Reduction"
    BLOCKCHAIN_APPLICATIONS = "Blockchain and Distributed Ledger Technologies"
    AI_ML_IMPLEMENTATION = "Artificial Intelligence and Machine Learning Applications"
    IOT_INTEGRATION = "Internet of Things (IoT) Integration"
    HEALTH_WELLNESS = "Employee Health and Wellness Programs"
    CRISIS_MANAGEMENT = "Crisis Management and Business Continuity"
    BRAND_REPUTATION = "Brand Reputation and Management"
    PRODUCT_INNOVATION = "Product Innovation and Development"
    GLOBALIZATION_STRATEGIES = "Globalization and International Expansion"
    ETHICAL_BUSINESS = "Ethical Business Practices and Corporate Social Responsibility"
    PRICING_STRATEGIES = "Pricing Strategies and Revenue Models"
    CUSTOMER_RETENTION = "Customer Retention and Loyalty Programs"
    AGILE_METHODOLOGIES = "Agile and Lean Methodologies in Business"
    MERGERS_ACQUISITIONS = "Mergers, Acquisitions, and Strategic Partnerships"
    GREEN_TECH = "Green Technologies and Renewable Energy"
...
# Function to extract images from PDF using PyMuPDF (Fitz)
def extract_images_from_pdf(pdf_path):
    ...
    for page_number in range(len(doc)):
        ...
        for img in images:
            ...
            image_filename = f"image_page_{page_number}_img_{xref}.{image_ext}"
            image_list.append(image_filename)
            with open(image_filename, "wb") as f:
                f.write(image_bytes)
    return image_list

# Function to process the extracted text into chunks for further use (e.g., embeddings)
def get_text_chunks(text):
    text_splitter = RecursiveCharacterTextSplitter(chunk_size=10000, chunk_overlap=1000)
    chunks = text_splitter.split_text(text)
    return chunks

# Function to store text chunks in FAISS vector store for semantic search
...
def provide_planning_and_solutions(analysis, budget=None, workforce=None):
    budget_info = f"Budget: ${budget}" if budget is not None else "No budget information provided."
    workforce_info = f"Workforce: {workforce} employees" if workforce is not None else "No workforce information provided."
    ...
    **Your plan should cover the following sections:**
    ...
    10. **Human Resources & Talent Management**:
        ...
        - Propose initiatives for improving employee satisfaction and retention.
    ...
    15. **Implementation Timeline & Key Milestones**:
        - Provide a detailed implementation plan with timelines, resource allocation, and key milestones for tracking progress.
    ...
    17. **Mergers & Acquisitions (M&A) Opportunities**:
        - Explore potential opportunities for mergers, acquisitions, or strategic partnerships.
        - Include a cost-benefit analysis and the strategic rationale for pursuing M&A activities.

    18. **Brand Evolution & Marketing Strategy**:
        - Suggest strategies for evolving the company’s brand identity.
        - Discuss methods for expanding brand recognition and improving customer engagement across different channels.

    19. **Pricing Strategy & Revenue Model**:
        - Assess the company's current pricing strategy and suggest improvements or alternatives.
        - Propose new revenue streams or business models for long-term sustainability.

    20. **Cultural Development & Organizational Improvement**:
        - Provide a plan for shaping and improving the company’s organizational culture.
        - Suggest leadership development and team-building activities to foster a strong internal culture.

    21. **Supply Chain Resilience**:
        - Analyze the resilience of the supply chain and propose strategies to strengthen it against disruptions.

    22. **Digital Transformation Assessment**:
        - Evaluate the current state of the company's digital initiatives and provide a roadmap for future improvements.

    23. **Data Analytics Capabilities**:
        - Assess the company’s capabilities in data analytics and suggest improvements for data-driven decision-making.

    24. **Sustainability and Corporate Social Responsibility (CSR) Initiatives**:
        - Discuss initiatives that promote sustainability and corporate social responsibility.

    25. **Intellectual Property Strategy**:
        - Outline strategies for protecting and leveraging intellectual property assets.

    26. **Regulatory Compliance Roadmap**:
        - Provide a roadmap for ensuring compliance with relevant laws and regulations.

    27. **Stakeholder Analysis**:
        - Analyze key stakeholders and their influence on the company’s success.

    28. **Customer Experience Enhancement**:
        - Suggest strategies for improving customer experience and engagement.

    29. **Social Media and Online Presence Analysis**:
        - Evaluate the company's current online presence and propose strategies for enhancement.

    30. **Crisis Management and Business Continuity**:
        - Detail plans for managing crises and ensuring continuity of operations.

    31. **Appendices** (if necessary):
        - Include any additional information, supporting data, or resources that further enhance the plan.
...
def generate_new_business_plan(business_idea, business_type, target_market, initial_investment, industry, location, product_positioning, usp, target_market_share, founders_experience, currency):
    sections = [
        "Executive Summary",
        "Company Description",
    ...
    for section in sections:
        ...
        Target Market Share in {location}: {target_market_share}% in 3 years
    ...
    """
        ...
        # Call your language model API with the constructed prompt
        response = generate_chatgpt_response(prompt)
        
        # Append the generated text to the full plan
        full_plan += f"# {section}\n\n{response}\n\n"
...
def generate_research_prompt(category: ResearchCategory, specific_query: str) -> str:
    base_prompt = f"Conduct a thorough research and analysis on the following {category.value} query:\n\n{specific_query}\n\n"
    
    category_specific_instructions = {
        ResearchCategory.CASE_STUDIES: "Provide recent case studies, analyze failures, and highlight successful pivots.",
        ResearchCategory.IP_LANDSCAPE: "Include patent research, infringement risks, and trademark analysis.",
        ResearchCategory.GEO_CULTURAL: "Analyze regional success rates, cultural impacts, and regulatory environments.",
        ResearchCategory.MARKET_INSIGHTS: "Analyze market size, growth trends, consumer behavior, and segmentation.",
        ResearchCategory.COMPETITIVE_INTEL: "Identify key competitors, their strategies, market share, and competitive advantages along with their SWOT analysis.",
        ResearchCategory.EMERGING_TECH: "Explore cutting-edge technologies, potential disruptors, and adoption trends.",
        ResearchCategory.ECONOMIC_POLITICAL: "Assess economic indicators, political climate, and policy impacts on business for both Political and Economical current trends.",
        ResearchCategory.SUSTAINABILITY: "Evaluate environmental impacts, sustainable practices, and green market opportunities.",
        ResearchCategory.FUNDING_INVESTMENT: "Analyze funding sources, investment trends, and financial market conditions.",
        ResearchCategory.WORKFORCE_TALENT: "Examine labor market trends, skill gaps, and talent acquisition strategies.",
        ResearchCategory.REGULATORY_COMPLIANCE: "Review relevant regulations, compliance requirements, and upcoming legislative changes.",
        ResearchCategory.CUSTOMER_EXPERIENCE: "Analyze customer satisfaction metrics, service trends, and experience optimization strategies.",
        ResearchCategory.SUPPLY_CHAIN: "Evaluate supply chain efficiencies, risks, and optimization opportunities.",
        ResearchCategory.CROSS_INDUSTRY: "Identify cross-industry applications, synergies, and potential partnerships.",
        ResearchCategory.LONG_TERM_TRENDS: "Forecast long-term industry trends, potential disruptions, and future scenarios.",
        ResearchCategory.DIGITAL_TRANSFORMATION: "Assess digital maturity, transformation strategies, and technology adoption rates.",
        ResearchCategory.CYBERSECURITY: "Analyze current threats, security measures, and best practices in data protection.",
        ResearchCategory.SOCIAL_MEDIA_TRENDS: "Examine platform usage, content strategies, and engagement metrics across industries.",
        ResearchCategory.DIVERSITY_INCLUSION: "Evaluate diversity initiatives, inclusion strategies, and their impact on business performance.",
        ResearchCategory.REMOTE_WORK: "Analyze remote work trends, productivity impacts, and tools for distributed team management.",
        ResearchCategory.CIRCULAR_ECONOMY: "Explore circular business models, waste reduction strategies, and resource efficiency innovations.",
        ResearchCategory.BLOCKCHAIN_APPLICATIONS: "Investigate blockchain use cases, implementation challenges, and potential industry disruptions.",
        ResearchCategory.AI_ML_IMPLEMENTATION: "Assess AI/ML adoption rates, application areas, and impact on business processes.",
        ResearchCategory.IOT_INTEGRATION: "Examine IoT implementation strategies, data utilization, and connected device ecosystems.",
        ResearchCategory.HEALTH_WELLNESS: "Analyze workplace wellness programs, health tech innovations, and their impact on productivity.",
        ResearchCategory.CRISIS_MANAGEMENT: "Evaluate crisis response strategies, business continuity plans, and risk mitigation approaches.",
        ResearchCategory.BRAND_REPUTATION: "Assess brand perception, reputation management strategies, and crisis communication tactics.",
        ResearchCategory.PRODUCT_INNOVATION: "Explore innovation methodologies, R&D strategies, and new product development processes.",
        ResearchCategory.GLOBALIZATION_STRATEGIES: "Analyze international expansion strategies, market entry tactics, and cross-border challenges.",
        ResearchCategory.ETHICAL_BUSINESS: "Examine ethical business practices, corporate social responsibility initiatives, and their market impact.",
        ResearchCategory.PRICING_STRATEGIES: "Analyze pricing models, revenue optimization strategies, and value-based pricing approaches.",
        ResearchCategory.CUSTOMER_RETENTION: "Evaluate customer loyalty programs, retention metrics, and strategies for reducing churn.",
        ResearchCategory.AGILE_METHODOLOGIES: "Assess the adoption of agile and lean practices, their impact on project management, and organizational agility.",
        ResearchCategory.MERGERS_ACQUISITIONS: "Examine M&A trends, due diligence processes, and post-merger integration strategies.",
        ResearchCategory.GREEN_TECH: "Investigate renewable energy innovations, green tech adoption rates, and their impact on sustainability efforts.",
    }
    
    base_prompt += category_specific_instructions.get(category, "")
    
    base_prompt += """
    General Instructions:
    1. Provide detailed, well-researched information from reliable sources.
    2. Include relevant statistics, data points, and expert opinions where applicable.
    3. Analyze trends and patterns in the data.
    4. Offer insights on potential implications for the business.
    5. Suggest actionable recommendations based on the findings.
    6. Cite sources for all major claims and data points.
    """
    
    return base_prompt

def generate_follow_up_answer(question, context, research_category=None):
    if research_category:
        prompt = generate_research_prompt(research_category, question)
    else:
        prompt = f"""
...
def display_follow_up_questions(context):
    ...
    research_category_key = f"{base_key}_research_category"
    research_query_key = f"{base_key}_research_query"
    submit_research_key = f"{base_key}_submit_research"
    question_type_key = f"{base_key}_question_type"
    ...
    question_type = st.radio("Select question type:", ["Complex", "Research"], key=question_type_key)
    
    if question_type == "Research":
        research_category = st.selectbox("Select Research Category", list(ResearchCategory), key=research_category_key)
        question = st.text_area("Enter your research query:", key=research_query_key)
        if st.button("Submit Research Request", key=submit_research_key):
            with st.spinner("Conducting research..."):
                answer, confidence = cached_research(research_category, question)
                st.write(f"Answer: {answer}")
                st.write(f"Confidence Level: {confidence*100:.2f}%")
                st.session_state.conversation_history.append(("User", question))
                st.session_state.conversation_history.append(("AI", f"Answer: {answer}\nConfidence Level: {confidence*100:.2f}%"))
        ...
        if st.button("Submit Question", key=submit_question_key):
            with st.spinner("Generating answer..."):
                if question_type == "Simple":
                    answer, confidence = generate_follow_up_answer(question, context)
                else:  # Complex
                    answer, confidence = generate_complex_answer(question, context)
                st.write(f"Answer: {answer}")
                st.write(f"Confidence Level: {confidence*100:.2f}%")
